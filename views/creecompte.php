...
if (isset($_POST['action']) && $_POST['action'] === 'login') {
    ...
    if (empty($errorsLogin)) {
        try {
            ...
            // Récupération du rôle en plus des autres champs
            $stmt = $pdo->prepare("SELECT id, prenom, password, role FROM utilisateurs WHERE email = ?");
            $stmt->execute([$email]);
            $user = $stmt->fetch();
